...
function StatsPage() {
  ...
  const tc = useCallback((key: string) => t(`pages.manager.statistics.${key}`), [t]);
  ...
  const formatedExtended = useMemo(() => {
    ...
    return {
      ...
      predictions: extended.predictions ? {
        ...
        rrevenueForecast7Days: parseFloat(extended.predictions.revenueForecast7Days.toFixed(2)),
        revenueForecast30Days: parseFloat(extended.predictions.revenueForecast30Days.toFixed(2)),
        occupancyForecast7Days: parseFloat(extended.predictions.occupancyForecast7Days.toFixed(2)),
        occupancyForecast30Days: parseFloat(extended.predictions.occupancyForecast30Days.toFixed(2)),
      } : undefined,
      trends: {
        ...extended.trends,
        revenueGrowthPercent: parseFloat(extended.trends.revenueGrowthPercent.toFixed(2)),
        occupancyGrowthPercent: parseFloat(extended.trends.occupancyGrowthPercent.toFixed(2)),
        bookingGrowthPercent: parseFloat(extended.trends.bookingGrowthPercent.toFixed(2))
      },
      seasonality: {
        ...extended.seasonality,
        weekdayVsWeekend: extended.seasonality.weekdayVsWeekend ? {
          weekdayAvgOccupancy: parseFloat(extended.seasonality.weekdayVsWeekend.weekdayAvgOccupancy.toFixed(2)),
          weekendAvgOccupancy: parseFloat(extended.seasonality.weekdayVsWeekend.weekendAvgOccupancy.toFixed(2)),
          difference: parseFloat(extended.seasonality.weekdayVsWeekend.difference.toFixed(2)),
        } : undefined,
            ...
            <Tooltip
              title={
                <Box>
                  ...
                  <Typography variant="body2">
                    {tc('accuracyRevenue')}: {formatNumber(mlPredict.accuracyRevenue, 2)}%
                  </Typography>
                  <Typography variant="body2">
                    {tc('accuracyOccupancy')}: {formatNumber(mlPredict.accuracyOccupancy, 2)}%
                  </Typography>
                </Box>
              }
            >
                  ...
                  <CardContent>
                    <Box display="flex" alignItems="center" justifyContent="space-between" mb={2}>
                      <Typography variant="h6" fontWeight={600}>
                        {tc('forecastPredictions') || 'Revenue & Occupancy Forecasts'}
                      </Typography>
                    </Box>
                    <Grid container spacing={2}>
                      <Grid size={{ xs: 12, sm: 6, md: 3 }}>
                        <Card sx={{ p: 2, backgroundColor: 'background.default', borderRadius: 2 }}>
                          ...
                          <Typography variant="h5" fontWeight="bold">
                            ${formatNumber(formatedExtended.predictions.revenueForecast7Days, 2)}
                          </Typography>
                        </Card>
                      </Grid>
                      <Grid size={{ xs: 12, sm: 6, md: 3 }}>
                        <Card sx={{ p: 2, backgroundColor: 'background.default', borderRadius: 2 }}>
                          ...
                          <Typography variant="h5" fontWeight="bold">
                            ${formatNumber(formatedExtended.predictions.revenueForecast30Days, 2)}
                          </Typography>
                        </Card>
                      </Grid>
                      <Grid size={{ xs: 12, sm: 6, md: 3 }}>
                        <Card sx={{ p: 2, backgroundColor: 'background.default', borderRadius: 2 }}>
                          <Box display="flex" alignItems="center" gap={1} mb={1}>
                            ...
                            <Typography variant="subtitle2" color="text.secondary">
                              {tc('occupancy7Days') || '7-Day Occupancy Forecast'}
                            </Typography>
                          </Box>
                          <Typography variant="h5" fontWeight="bold">
                            {formatNumber(formatedExtended.predictions.occupancyForecast7Days, 2)}%
                          </Typography>
                        </Card>
                      </Grid>
                      <Grid size={{ xs: 12, sm: 6, md: 3 }}>
                        <Card sx={{ p: 2, backgroundColor: 'background.default', borderRadius: 2 }}>
                          <Box display="flex" alignItems="center" gap={1} mb={1}>
                            <PeopleAltOutlined color="success" />
                            <Typography variant="subtitle2" color="text.secondary">
                              {tc('occupancy30Days') || '30-Day Occupancy Forecast'}
                            </Typography>
                          </Box>
                          <Typography variant="h5" fontWeight="bold">
                            {formatNumber(formatedExtended.predictions.occupancyForecast30Days, 2)}%
                          </Typography>
                        </Card>
                      </Grid>
                    </Grid>
            ...
            <Grid size={{ xs: 12 }}>
              <Card sx={{ borderRadius: 3 }}>
                <CardContent>
                  ...
                  <Grid container spacing={2}>
                    {[
                      { label: tc('revenueGrowth'), value: formatedExtended.trends.revenueGrowthPercent },
                      { label: tc('occupancyGrowth'), value: formatedExtended.trends.occupancyGrowthPercent },
                      { label: tc('bookingGrowth'), value: formatedExtended.trends.bookingGrowthPercent },
                    ].map((tItem) => (
                ...
                <Card sx={{ borderRadius: 3 }}>
                  <CardContent>
                    <Typography variant="h6" gutterBottom fontWeight={600}>
                      {tc('weekdayVsWeekend') || 'Weekday vs Weekend Comparison'}
                    </Typography>
                    <Grid container spacing={2}>
                      <Grid size={{ xs: 12, sm: 4 }}>
                        <Card sx={{ p: 2, backgroundColor: 'background.default', borderRadius: 2, textAlign: 'center' }}>
                          <CalendarToday color="primary" fontSize="large" />
                          <Typography variant="subtitle1" mt={1}>
                            {tc('weekdayOccupancy') || 'Weekday Avg'}
                          </Typography>
                          <Typography variant="h5" fontWeight="bold">
                            {formatNumber(formatedExtended.seasonality.weekdayVsWeekend.weekdayAvgOccupancy, 2)}%
                          </Typography>
                        </Card>
                      </Grid>
                      <Grid size={{ xs: 12, sm: 4 }}>
                        <Card sx={{ p: 2, backgroundColor: 'background.default', borderRadius: 2, textAlign: 'center' }}>
                          ...
                          <Typography variant="h5" fontWeight="bold">
                            {formatNumber(formatedExtended.seasonality.weekdayVsWeekend.weekendAvgOccupancy, 2)}%
                          </Typography>
                        </Card>
                      </Grid>
                      <Grid size={{ xs: 12, sm: 4 }}>
                        <Card sx={{ p: 2, backgroundColor: 'background.default', borderRadius: 2, textAlign: 'center' }}>
                          {formatedExtended.seasonality.weekdayVsWeekend.difference >= 0 ? (
                            <TrendingUp color="success" fontSize="large" />
                          ) : (
                            <TrendingDown color="error" fontSize="large" />
                          )}
                          ...
                          <Typography variant="h5" fontWeight="bold">
                            {formatNumber(formatedExtended.seasonality.weekdayVsWeekend.difference, 2)}%
                          </Typography>
                        </Card>
                      </Grid>
                    </Grid>
              ...
              <Card sx={{ borderRadius: 3, height: '100%' }}>
                <CardContent>
                  ...
                  <ResponsiveContainer width="100%" height={350}>
                    <BarChart data={formatedExtended.seasonality.peakDaysOfWeek}>
                      <CartesianGrid strokeDasharray="3 3" />
                      ...
                      <ChartTooltip />
                      <Legend />
                      <Bar yAxisId="left" dataKey="avgOccupancy" fill="#1976d2" radius={[8, 8, 0, 0]} name="Avg Occupancy %" />
                      <Bar yAxisId="right" dataKey="avgRevenue" fill="#2e7d32" radius={[8, 8, 0, 0]} name="Avg Revenue ($)" />
                    </BarChart>
                  </ResponsiveContainer>
                </CardContent>
              </Card>

...
function GuestLayout() {
  const [currentPage, setCurrentPage] = useState<PageState>(
    () => (localStorage.getItem("guestCurrentPage") as PageState) || "Available Services"
  );

  useEffect(() => {
    localStorage.setItem("guestCurrentPage", currentPage);
  }, [currentPage]);
    ...
    <div style={{ display: "flex", gap: "1rem" }}>
      <Box sx={{ width: { xs: "100%", md: "70%" } }}>
        <GuestNavbar
          ...
          subpages={subpages}
        />
        {currentPage === "Available Services" && <AvailableServicesPage />}
        {currentPage === "Booked Services" && <BookedServicesPage />}
        {currentPage === "Book Hotel Room" && <HotelBookingPage />}
      </Box>

...
function ServiceItem ({item, index}: {item: RequestedServiceProps, index: number}) {
  const [openRate, setOpenRate] = useState(false);
  const [openCancel, setOpenCancel] = useState(false);

  return (
    <div key={index} style={{display: 'flex', marginTop: '10px', padding: '15px', background: 'white'}}>
      <RateServiceDialog open={openRate} setOpen={setOpenRate} scheduleId={item.id} />
      <CancelServiceDialog open={openCancel} setOpen={setOpenRate} scheduleId={item.id} />
      ...
      <div style={{display: 'flex', flexDirection: 'column', justifyContent: 'space-between', width: '100%', padding: '5px 20px'}}>
        ...
        <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'end'}}>
          ...
          {(item.status === "IN_PROGRESS" || item.status === "PENDING") ? (
            <Button disabled={(new Date(item.datetime).getTime() - Date.now() < 24 * 60 * 60 * 1000) || item.status === "IN_PROGRESS"} onClick={() => setOpenCancel(true)}>Cancel</Button>
          ) : (
            <Button disabled={item.status === "CANCELED"} onClick={() => setOpenRate(true)}>Rate</Button>
          )}
          </div>
      </div>
    </div>

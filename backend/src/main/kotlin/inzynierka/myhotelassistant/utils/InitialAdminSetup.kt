...
class InitialAdminSetup(
    ...
    private fun setupAdminInteractively() {
        ...
        fun promptInput(fieldName: String, regex: Regex, description: String): String {
            var input: String
            while (true) {
                print("Enter $fieldName ($description): ")
                input = scanner.nextLine().trim()
                if (regex.matches(input)) break
                else println("Invalid $fieldName, please try again.")
            }
            return input
        }

        val username = promptInput(
            fieldName = "username",
            regex = "^[\\w-_]{4,20}$".toRegex(),
            description = "between 4 and 20 alphanumerical characters with '_' or '-'"
        )
        val password = promptInput(
            fieldName = "password",
            regex = "^.{8,}$".toRegex(),
            description = "at least 8 characters long"
        )
        val email = promptInput(
            fieldName = "email",
            regex = "^[\\w-.]+@([\\w-]+\\.)+[\\w-]{2,4}$".toRegex(),
            description = "valid email format"
        )
        val firstName = promptInput(
            fieldName = "first name",
            regex = "^[a-zA-Z-]{2,20}$".toRegex(),
            description = "max 20 letters"
        )

        val lastName = promptInput(
            fieldName = "last name",
